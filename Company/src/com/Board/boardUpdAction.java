...
public class boardUpdAction implements Action {
	Logger logger = Logger.getLogger(boardUpdAction.class);
	@Override
	public ActionForward execute(HttpServletRequest req, HttpServletResponse res) throws IOException, ServletException {
		...
		if ( result == 1 ){
			forward.setPath("./boardUpdateSucess.jsp");
		}
		else {
			boardLogic.hitUpdate(pMap);
			forward.setPath("./boardUpdateFail.jsp");
		}
		return forward;	
	}
